        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                if (option == 1)
                {
                    ...
                    currentCinema.WatchMovie(currentCinema.ListOfMovies.FirstOrDefault(x => x.Title.ToLower() == chosenMovie.ToLower()));
                }
                else if (option == 2)
                {
                    Console.WriteLine("Enter your favorite genre from list");
                    foreach (Genre genre in Enum.GetValues(typeof(Genre)))
                    {
                        Console.WriteLine(genre);
                    }
                    string userGenre = Console.ReadLine();
                    if (Enum.TryParse<Genre>(userGenre, out Genre genres))
                    {
                        List<Movie> list = currentCinema.ListOfMovies.Where(x => x.Genre.ToString() == genres.ToString()).ToList();
                        Console.WriteLine($"All {genres.ToString()} movies");
                        ...
                        string chosenMovie = Console.ReadLine();
                        currentCinema.WatchMovie(list.FirstOrDefault(x => x.Title.ToLower() == chosenMovie.ToLower()));
                    }
                    else
                    {
                        Console.WriteLine("You must choose valid genre");
                    }
                }
                else
                {
            ...
            {
                Console.WriteLine(e.Message);
                Console.WriteLine("Movies does not exist");
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
